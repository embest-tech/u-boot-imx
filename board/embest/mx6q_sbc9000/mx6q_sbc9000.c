...
#define USBCMD 0x140
	...
	/* board id for linux */
	gd->bd->bi_arch_number = MACH_TYPE_MX6Q_SBC9000;
...
int mx6_rgmii_rework(char *devname, int phy_addr)
{
	...
	printf("----mx6_rgmii_rework: phy_addr= %d\n", phy_addr);


	ret = phy_read(devname, phy_addr, MII_PHYSID1, &id1);
	ret = phy_read(devname, phy_addr, MII_PHYSID2, &id2);
	printf("----id= 0x%x%x\n", id1, id2);

	/******************************************************/
	/* To enable AR8031 ouput a 125MHz clk from CLK_25M */
	ret = phy_write(devname, phy_addr, 0xd, 7);
	ret = phy_write(devname, phy_addr, 0xe, 0x8016);
	ret = phy_write(devname, phy_addr, 0xd, 0x4007);
	ret = phy_read(devname, phy_addr, 0xe, &val);
	val &= 0xffe3;
	val |= 0x18;
	ret = phy_write(devname, phy_addr, 0xe, val);

	/* Introduce tx clock delay */
	ret = phy_write(devname, phy_addr, 0x1d, 0x5);
	...
	if(val & BMCR_PDOWN)
		ret = phy_write(devname, phy_addr, 0x0, (val & ~BMCR_PDOWN));
...
void enet_board_init(void)
{
    ...
    /* phy reset: gpio3-31 */
    set_gpio_output_val(GPIO3_BASE_ADDR, (1 << 31), 0);
